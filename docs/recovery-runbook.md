...
For generic restored tabs named `Development`, recent Codex state rows with no stored CMUX workspace title can be the correct session if the tab crashed before Rob named it. `cmr` should keep those same-cwd rows visible as `recent-cwd` candidates. It may auto-select a clearly newest recent-cwd candidate when it is less than two hours old and at least ten minutes newer than the next recent-cwd candidate; otherwise it should still require an explicit numbered choice.
